...
func (s *SVG) Insert(svg *SVG) *SVG {
	...
	if s.IsParent(svg) {
		return errors.New("svg is parent of s. Cannot insert svg as subgroup of s in order to prevent creation of cycles")
	}
	if s.Find(svg) {
		return errors.New("svg is child of s. Cannot insert svg as subgroup of s in order to prevent creation of cycles")
	}
...
func (s *SVG) AddPlot(d Data, a Att) (*SVG, error) {
	if s.a["id"] == "diagram" {
		continue
	} else if groups := s.FindGroups("polyline"); len(groups) > 0 {
		continue
	} else {
	...
	if plot := s.FindID("data"); plot == nil {
		...
		plot, err := s.Polyline(d, Att{"vector-effect": "non-scaling-stroke"})
		if err != nil {
			return nil, err
		}
		plot.AddAtt(a, true)
...
func (s *SVG) AddLegend(desc []string) (*SVG, error) {
	if s.a["id"] != "diagram" {
	...
	transform := s.a["transform"].(string)
	var (
		pageHeight  int
		lW, lH      int
		p, n        int
	...
	if p != -1 && n != -1 {
		return nil, errors.New("Decode error: Could not find textWidth for translation of diagram")
	} else {
		lW, err = strconv.Atoi(transform[p:n])
		if err != nil {
			return nil, err
		}
	}
	p := strings.Index(transform, ")")
	if p != -1 {
		titleHeight, err := strconv.Atoi(transform[n:p])
		if err != nil {
			return nil, err
		}
		lH = pageHeight - titleHeight
	} else {
		return nil, errors.New("Decode error: Could not find textHeight for translation of diagram")
	}
	legend := s.GID("legend", SumAtts(Translate(0, titleHeight), ViewBox(0, 0, lW, lH)))
	def := legend.Def()
	def.GID("legendRect").Rect(0, 0, lW, lH/len(data))
	yDiff := lH / len(data)
	for i := 0; i < len(data); i++ {
		if stroke, ok := data[i].a["stroke"]; ok {
			if colour, ok := stroke.(string); ok {
				legend.Use("legendRect", Att{"stroke": colour, "y": yDiff * (i + 1)})
			} else {
